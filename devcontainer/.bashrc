...
function _update_ps1() {
    PS1="$($GOPATH/bin/powerline-go -error $? -newline -modules venv,user,host,ssh,cwd,perms,git,hg,jobs,exit )"
}
if [ "$TERM" != "linux" ] && [ -f "$GOPATH/bin/powerline-go" ]; then
    PROMPT_COMMAND="_update_ps1; $PROMPT_COMMAND"
fi
# CodeArtifact Authorization
if [ -n "$CODEARTIFACT_DOMAIN" ] && [ -n "$CODEARTIFACT_DOMAIN_OWNER" ]; then
